...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:123456@localhost/driver_behavior?charset=utf8mb4'
...
def get_speed(driver_id):
    ...
    # 按时间正序排序，分页
    total = query.count()
    records = query.order_by(RawDrivingRecord.Time).offset((page-1)*page_size).limit(page_size).all()
...
def get_driver_dates(driver_id):
    ...
    dates = db.session.query(
        func.date(RawDrivingRecord.Time).label('record_date')
    ).filter(RawDrivingRecord.driverID == driver_id).distinct().order_by('record_date').all()
